...
public class PlayerScript : MonoBehaviour
{
    ...
    bool isGrounded()
    {
        return Physics.Raycast(transform.position + transform.forward * 0.4f + transform.up *0.1f,
            Vector3.down,0.1f); 
    }
    void TurnLeft()
    {
    ...
    IEnumerator LightControl()
    {
        while (true)
        {
            leftLight.intensity = rightLight.intensity = 3f;
        ...
        if (!LeanTween.isTweening(gameObject))
        {
            ...
            for (int i = 0; i < 5; i++)
            {
                ...
                GameObject temp = Instantiate(missile, origin,
                    Quaternion.AngleAxis(dir==Direction.RIGHT ? 0f : 180f,Vector3.up));

                Vector3 tarPos = missilePoint.position + missilePoint.forward * 20 +
                    missilePoint.up * Random.Range(-1,1f);
